...
async def contributor_legacy_narration():
    ...
    users_list = data.get("legacy", [])
    if len(users_list) >= 2:
        for p in ("mentor", "coach", "teacher"):
            peaks = []
            for u in users_list:
                dt = u.get("drift_timeline", [])
                vals = [(e.get("weights", {}).get(p, 0)) for e in dt if e.get("weights", {}).get(p, 0) > 0]
                if vals:
                    peaks.append((u["user_id"], max(vals)))
            if len(peaks) >= 2:
                peaks.sort(key=lambda x: x[1], reverse=True)
                top, second = peaks[0], peaks[1]
                if top[1] > second[1]:
                    sentences.append(f"{top[0]}'s {p} influence peaked {top[1]} which is higher than {second[0]}'s at {second[1]}.")
                elif top[1] == second[1]:
...
@app.get("/api/confidence-health/export")
async def confidence_health_export():
    ...
    ov = data.get("overall", {})
    lines = [
        f"youfeel Confidence Health Report — {now_str}",
        f"Trend: {data.get('trend', 'unknown')}",
        f"Overall delta: {data.get('overall_delta', 'N/A')}",
        f"Average: {ov.get('avg', 'N/A')}%",
        f"Spread: {ov.get('spread', 'N/A')}",
        f"Healthy ratio: {ov.get('healthy_ratio', 0) * 100}%",
        f"Status: {ov.get('status', 'unknown')}",
        f"Total interactions: {ov.get('total_interactions', 0)}",
        "",
        "=== SESSION BREAKDOWN ===",
    ]
    for i, s in enumerate(sessions):
        lines.append(f"  Session {s['window']}: avg={s['avg']}%, spread={s['spread']}, healthy={s['healthy_ratio']*100}%, status={s['status']}")
    if comps:
        lines.extend(["", "=== COMPARISONS ==="])
        for c in comps:
            lines.append(f"  {c['from_window']} -> {c['to_window']}: avg delta={c['avg_delta']:+.1f}%, spread delta={c['spread_delta']:+.1f}")
    lines.extend(["", f"Generated by youfeel — {now_str}"])
    text = "\n".join(lines)
    return Response(
        content=text,
